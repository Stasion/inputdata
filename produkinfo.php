...
class cproduk extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('produk', 'produk', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// nama_produk
		$this->nama_produk = new cField('produk', 'produk', 'x_nama_produk', 'nama_produk', '`nama_produk`', '`nama_produk`', 200, -1, FALSE, '`nama_produk`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->nama_produk->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['nama_produk'] = &$this->nama_produk;

		// deskripsi
		$this->deskripsi = new cField('produk', 'produk', 'x_deskripsi', 'deskripsi', '`deskripsi`', '`deskripsi`', 201, -1, FALSE, '`deskripsi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['deskripsi'] = &$this->deskripsi;

		// id_pelaku_kreatif
		$this->id_pelaku_kreatif = new cField('produk', 'produk', 'x_id_pelaku_kreatif', 'id_pelaku_kreatif', '`id_pelaku_kreatif`', '`id_pelaku_kreatif`', 3, -1, FALSE, '`id_pelaku_kreatif`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// harga
		$this->harga = new cField('produk', 'produk', 'x_harga', 'harga', '`harga`', '`harga`', 200, -1, FALSE, '`harga`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['harga'] = &$this->harga;

		// foto_produk
		$this->foto_produk = new cField('produk', 'produk', 'x_foto_produk', 'foto_produk', '`foto_produk`', '`foto_produk`', 200, -1, TRUE, '`foto_produk`', FALSE, FALSE, FALSE, 'IMAGE');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("produkview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("produkview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "produkadd.php?" . $this->UrlParm($parm);
		else
			return "produkadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("produkedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("produkadd.php", $this->UrlParm($parm));
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->id_pelaku_kreatif->Exportable) $Doc->ExportCaption($this->id_pelaku_kreatif);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->id_pelaku_kreatif->Exportable) $Doc->ExportField($this->id_pelaku_kreatif);

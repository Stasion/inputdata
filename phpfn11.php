 ...
 * (C) 2002-2014 e.World Technology Limited. All rights reserved.
*/
...
function CurrentProjectID() {
	...
	return "{D322CAA1-B5DF-4315-A18C-262A6180EE7B}";
}
...
class cBreadcrumb {
	...
	function __construct() {
		global $Language;
		$this->Links[] = array("home", "HomePage", "home.php", "ewHome", "", FALSE); // Home
	}
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						$this->setSessionUserID($rs->fields('id')); // Load User ID
						if (is_null($rs->fields('role'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('role'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if ($RsUser = $conn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('id'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}

...
$produk_kreatif_foto_produk_blobview = NULL; // Initialize page object first

class cproduk_kreatif_foto_produk_blobview extends cproduk_kreatif {
	...
	// Page object name
	var $PageObjName = 'produk_kreatif_foto_produk_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'produk_kreatif');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('produk_kreatif', 'x_foto_produk');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('foto_produk');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('foto_produk')));
				if (trim(strval($this->Recordset->fields('foto_produk'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('foto_produk') . "\"");
				}
...
// Create page object
if (!isset($produk_kreatif_foto_produk_blobview)) $produk_kreatif_foto_produk_blobview = new cproduk_kreatif_foto_produk_blobview();

// Page init
$produk_kreatif_foto_produk_blobview->Page_Init();

// Page main
$produk_kreatif_foto_produk_blobview->Page_Main();
?>
<?php
$produk_kreatif_foto_produk_blobview->Page_Terminate();
?>

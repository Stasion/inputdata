...
//
// Table class for produk_kreatif
//
class cproduk_kreatif extends cTable {
	var $id;
	var $nama_produk;
	var $deskripsi;
	var $id_pelaku_kreatif;
	var $harga;
	var $foto_produk;
	...
	function __construct() {
		...
		$this->TableVar = 'produk_kreatif';
		$this->TableName = 'produk_kreatif';
		...
		// id
		$this->id = new cField('produk_kreatif', 'produk_kreatif', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// nama_produk
		$this->nama_produk = new cField('produk_kreatif', 'produk_kreatif', 'x_nama_produk', 'nama_produk', '`nama_produk`', '`nama_produk`', 200, -1, FALSE, '`nama_produk`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nama_produk'] = &$this->nama_produk;

		// deskripsi
		$this->deskripsi = new cField('produk_kreatif', 'produk_kreatif', 'x_deskripsi', 'deskripsi', '`deskripsi`', '`deskripsi`', 201, -1, FALSE, '`deskripsi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['deskripsi'] = &$this->deskripsi;

		// id_pelaku_kreatif
		$this->id_pelaku_kreatif = new cField('produk_kreatif', 'produk_kreatif', 'x_id_pelaku_kreatif', 'id_pelaku_kreatif', '`id_pelaku_kreatif`', '`id_pelaku_kreatif`', 3, -1, FALSE, '`id_pelaku_kreatif`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id_pelaku_kreatif->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id_pelaku_kreatif'] = &$this->id_pelaku_kreatif;

		// harga
		$this->harga = new cField('produk_kreatif', 'produk_kreatif', 'x_harga', 'harga', '`harga`', '`harga`', 200, -1, FALSE, '`harga`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['harga'] = &$this->harga;

		// foto_produk
		$this->foto_produk = new cField('produk_kreatif', 'produk_kreatif', 'x_foto_produk', 'foto_produk', '`foto_produk`', '`foto_produk`', 200, -1, TRUE, '`foto_produk`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['foto_produk'] = &$this->foto_produk;
	}
	...
	function getSqlSelect() { // Select
		return ($this->_SqlSelect <> "") ? $this->_SqlSelect : "SELECT produk.* FROM " . $this->getSqlFrom();
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("produk_kreatifview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("produk_kreatifview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "produk_kreatifadd.php?" . $this->UrlParm($parm);
		else
			return "produk_kreatifadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("produk_kreatifedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("produk_kreatifadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("produk_kreatifdelete.php", $this->UrlParm());
	}
   ...
   // Common render codes
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->nama_produk->Exportable) $Doc->ExportCaption($this->nama_produk);
					if ($this->deskripsi->Exportable) $Doc->ExportCaption($this->deskripsi);
					if ($this->harga->Exportable) $Doc->ExportCaption($this->harga);
					if ($this->foto_produk->Exportable) $Doc->ExportCaption($this->foto_produk);
				} else {
					if ($this->nama_produk->Exportable) $Doc->ExportCaption($this->nama_produk);
					if ($this->harga->Exportable) $Doc->ExportCaption($this->harga);
				}
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->nama_produk->Exportable) $Doc->ExportField($this->nama_produk);
						if ($this->deskripsi->Exportable) $Doc->ExportField($this->deskripsi);
						if ($this->harga->Exportable) $Doc->ExportField($this->harga);
						if ($this->foto_produk->Exportable) $Doc->ExportField($this->foto_produk);
					} else {
						if ($this->nama_produk->Exportable) $Doc->ExportField($this->nama_produk);
						if ($this->harga->Exportable) $Doc->ExportField($this->harga);
					}
	...
	function AddUserIDFilter($sFilter) {
		...
		if (!$this->UserIDAllow($id) && !$Security->IsAdmin()) {
			...
			if ($sFilterWrk <> "")
				$sFilterWrk = '`id_pelaku_kreatif` IN (' . $sFilterWrk . ')';
		}
	...
	function GetUserIDSubquery(&$fld, &$masterfld) {
		global $conn;
		$sWrk = "";
		$sSql = "SELECT " . $masterfld->FldExpression . " FROM produk";
	...
	function Row_Inserted($rsold, &$rsnew) {
		...
		//echo "Row Inserted"
		resizeImage("uploads/".$rsnew['foto_produk']);
	}

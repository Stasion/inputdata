...
$produk_edit = NULL; // Initialize page object first

class cproduk_edit extends cproduk {
	...
	function Page_Main() {
		...
		if ($this->id->CurrentValue == "")
			$this->Page_Terminate("produklist.php"); // Invalid key, return to list
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (!$GLOBALS["produk"]->UserIDAllow("edit")) $sWhereWrk = $GLOBALS["pelaku_kreatif"]->AddUserIDFilter($sWhereWrk);
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// id_pelaku_kreatif
			$this->id_pelaku_kreatif->SetDbValueDef($rsnew, $this->id_pelaku_kreatif->CurrentValue, 0, $this->id_pelaku_kreatif->ReadOnly);
...
// Create page object
if (!isset($produk_edit)) $produk_edit = new cproduk_edit();

// Page init
$produk_edit->Page_Init();

// Page main
$produk_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$produk_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var produk_edit = new ew_Page("produk_edit");
produk_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = produk_edit.PageID; // For backward compatibility

// Form object
var fprodukedit = new ew_Form("fprodukedit");
...
fprodukedit.Form_CustomValidate = 
...
fprodukedit.ValidateRequired = true;
<?php } else { ?>
fprodukedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fprodukedit.Lists["x_id_pelaku_kreatif"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_nama_pemilik","x_nama_industri_kreatif","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $produk_edit->ShowPageHeader(); ?>
<?php
$produk_edit->ShowMessage();
?>
<form name="fprodukedit" id="fprodukedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($produk_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $produk_edit->Token ?>">
<?php } ?>
...
<?php if ($produk->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_produk_id" for="x_id" class="col-sm-2 control-label ewLabel"><?php echo $produk->id->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $produk->id->CellAttributes() ?>>
<span id="el_produk_id">
<span<?php echo $produk->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $produk->id->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($produk->id->CurrentValue) ?>">
<?php echo $produk->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
fprodukedit.Lists["x_id_pelaku_kreatif"].Options = <?php echo (is_array($produk->id_pelaku_kreatif->EditValue)) ? ew_ArrayToJson($produk->id_pelaku_kreatif->EditValue, 1) : "[]" ?>;
</script>
</span>

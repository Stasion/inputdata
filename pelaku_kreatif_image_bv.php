...
$pelaku_kreatif_image_blobview = NULL; // Initialize page object first

class cpelaku_kreatif_image_blobview extends cpelaku_kreatif {
	...
	// Page object name
	var $PageObjName = 'pelaku_kreatif_image_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'pelaku_kreatif');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('pelaku_kreatif', 'x_image');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('image')));
...
// Create page object
if (!isset($pelaku_kreatif_image_blobview)) $pelaku_kreatif_image_blobview = new cpelaku_kreatif_image_blobview();

// Page init
$pelaku_kreatif_image_blobview->Page_Init();

// Page main
$pelaku_kreatif_image_blobview->Page_Main();
?>
<?php
$pelaku_kreatif_image_blobview->Page_Terminate();
?>
